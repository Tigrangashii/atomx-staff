...
export default defineEventHandler(async (event) => {
  ...
  if (!title)
    throw createError({
      statusCode: 400,
      statusMessage: "Titulli i dokumentit mungon.",
    });
  ...
  const [{ data: uploader }, { data: recipients, error: recipientsError }] =
    await Promise.all([
      admin
        .from("profiles")
  ...
  if (recipientsError)
    throw createError({
      statusCode: 500,
      statusMessage: recipientsError.message,
    });
  if (!["owner", "manager"].includes(uploader?.role || "")) {
    throw createError({
      statusCode: 403,
      statusMessage: "Nuk ke të drejtë të dërgosh këtë njoftim.",
    });
  }
  ...
  const rows = (recipients || []).map((recipient) => ({
    ...
    title: "Dokument i ri i kompanisë",
    message,
  }));
  if (rows.length) {
