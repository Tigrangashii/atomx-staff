...
export default defineEventHandler(async (event) => {
  ...
  if (!["approved", "rejected"].includes(body.status))
    throw createError({
      statusCode: 400,
      statusMessage: "Statusi nuk është valid.",
    });
  if (body.status === "rejected" && !body.rejectionReason?.trim())
    throw createError({
      statusCode: 400,
      statusMessage: "Arsyeja e refuzimit është e detyrueshme.",
    });
  ...
  if (!reviewer || !["owner", "manager"].includes(reviewer.role))
    throw createError({
      statusCode: 403,
      statusMessage: "Nuk ke të drejtë ta shqyrtosh këtë kërkesë.",
    });
  ...
  const { data: request, error: requestError } = await supabase
    .from("leave_requests")
    .select(
      "id, employee_id, leave_type, start_date, end_date, status, employee:profiles!leave_requests_employee_id_fkey(id, full_name, email)",
    )
    .eq("id", id)
    .single();
  if (requestError || !request)
    throw createError({
      statusCode: 404,
      statusMessage: "Kërkesa nuk u gjet.",
    });

  if (request.status !== "pending")
    throw createError({
      statusCode: 409,
      statusMessage: "Kjo kerkese eshte shqyrtuar me heret.",
    });
  ...
  if (employee?.email) {
    ...
    const subject = approved
      ? "Kërkesa juaj për pushim është aprovuar"
      : "Kërkesa juaj për pushim është refuzuar";
    const htmlContent = renderAtomxEmail({
      ...
      intro: approved
        ? "Kërkesa juaj për pushim është aprovuar nga menaxhmenti."
        : "Kërkesa juaj për pushim nuk u aprovua.",
    ...
    await admin.from("notifications").insert({
      ...
      title: subject,
      message: approved
        ? "Kërkesa juaj për pushim u aprovua."
        : `Kërkesa u refuzua: ${body.rejectionReason}`,
    });
    await sendBrevoEmail(event, {
      emailType: approved ? "leave_approved" : "leave_rejected",
