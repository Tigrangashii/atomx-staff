...
export default defineEventHandler(async (event) => {
  ...
  const fields = Object.fromEntries(
    (parts || [])
      ...
      .map((part) => [part.name, part.data?.toString() || ""]),
  ) as Record<string, string>;
  const certificate = (parts || []).find(
    (part) => part.name === "medicalCertificate" && part.filename,
  );
  ...
  if (!body.startDate || !body.endDate || body.endDate < body.startDate) {
    throw createError({
      statusCode: 400,
      statusMessage: "Periudha e pushimit nuk është valide.",
    });
  }
  if (body.leaveType === "sick" && !certificate?.data?.length) {
    throw createError({
      statusCode: 400,
      statusMessage: "Vërtetimi mjekësor është i detyrueshëm.",
    });
  }
  ...
  if (profileError || !profile)
    throw createError({
      statusCode: 400,
      statusMessage: profileError?.message || "Profili nuk u gjet.",
    });
  ...
  if (error || !request)
    throw createError({
      statusCode: 400,
      statusMessage: error?.message || "Kërkesa nuk u ruajt.",
    });
  ...
  if (certificate?.data?.length) {
    const safeName = (certificate.filename || "certificate").replace(
      /[^a-zA-Z0-9._-]/g,
      "-",
    );
    ...
    const { error: uploadError } = await admin.storage
      ...
      .upload(certificatePath, certificate.data, {
    ...
    const { error: pathError } = await admin
      .from("leave_requests")
      .update({ medical_certificate_path: certificatePath })
      .eq("id", request.id);
    if (pathError) {
      await admin.storage
        .from("medical-certificates")
  ...
  for (const reviewer of reviewers || []) {
    await admin.from("notifications").insert({
      user_id: reviewer.id,
      title: subject,
      message: `${name} ka kërkuar pushim nga ${body.startDate} deri më ${body.endDate}.`,
    });
    if (reviewer.email)
