...
export function renderAtomxEmail(options: {
  eyebrow: string;
  title: string;
  intro: string;
  content: string;
...
export async function sendBrevoEmail(event: any, input: EmailInput) {
  ...
  const dedupeKey = input.leaveRequestId
    ? `${input.emailType}:${input.leaveRequestId}:${recipient}`
    : null;
  ...
  if (dedupeKey) {
    ...
    if (existing?.status === "sent" || existing?.status === "pending") {
      return {
        status: existing.status as EmailLogStatus,
        errorMessage: null,
        skipped: true,
      };
    }
    ...
    if (existing?.status === "failed") {
      ...
      if (retryError || !retried)
        return {
          status: "failed",
          errorMessage:
            retryError?.message || "Email-i i njejte po procesohet.",
          skipped: true,
        };
      logId = retried.id;
    }
    ...
    if (!logId) {
      ...
      if (claimError?.code === "23505") {
        const { data: duplicate } = await admin
          ...
          .maybeSingle();
        return {
          status: (duplicate?.status || "pending") as EmailLogStatus,
          errorMessage: null,
          skipped: true,
        };
      }
      if (claimError || !claimed)
        throw createError({
          statusCode: 500,
          statusMessage:
            claimError?.message || "Email-i nuk mund te planifikohej.",
        });
      logId = claimed.id;
    }
  }
  ...
  if (!config.brevoApiKey || !senderEmail) {
    ...
    try {
      ...
      errorMessage =
        error?.data?.message || error?.message || "Dërgimi i email-it dështoi.";
    }
  }
