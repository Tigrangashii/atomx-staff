...
const filteredRequests = computed(() =>
  requests.value.filter((request) => {
    const employeeMatches =
      ...
      request.employee_id === filters.employeeId;
    const statusMatches =
...
async function load() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("leave_requests")
      .select(
        "id, employee_id, leave_type, start_date, end_date, reason, rejection_reason, status, employee:profiles!leave_requests_employee_id_fkey(full_name)",
      )
    ...
    if (role.value === "owner" || role.value === "manager") {
      ...
      employees.value = profile
        ? [{ id: authUser.id, full_name: profile.full_name }]
        : [];
    }
...
async function addHoliday() {
  ...
  const query = editingHolidayId.value
    ? supabase
        ...
        .eq("id", editingHolidayId.value)
    : supabase.from("official_holidays").insert({
        name: holidayForm.name.trim(),
        holiday_date: holidayForm.date,
  ...
  if (error) {
    errorMessage.value =
      error.code === "23505"
        ? "Për këtë datë ekziston tashmë një festë zyrtare."
        : error.message;
    return;
  }
...
<template>
  <section class="relative space-y-6">
    <div v-if="role !== 'user'" class="absolute right-0 top-12 z-10">
      <UButton
        label="Lista e festave zyrtare"
        icon="i-lucide-list"
        color="neutral"
        variant="soft"
        @click="holidayListModalOpen = true"
      />
    </div>
    <div v-if="role !== 'user'" class="absolute right-0 top-0 z-10">
      <UButton
        label="Shto festë zyrtare"
        icon="i-lucide-plus"
    ...
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div>
        ...
        <h2 class="text-2xl font-semibold text-highlighted">
          Kalendari i pushimeve
        </h2>
        <p class="muted">Shiko pushimet sipas datës, punëtorit dhe statusit.</p>
      </div>
      <UButton
        to="/leaves"
        label="Lista e kërkesave"
        icon="i-lucide-list"
        color="neutral"
        variant="soft"
      />
    ...
    <UCard>
      <div v-if="loading" class="space-y-3">
        <USkeleton class="h-10 w-full" /><USkeleton class="h-96 w-full" />
      </div>
      <div v-else class="overflow-x-auto">
        <FullCalendar :options="calendarOptions" />
      </div>
      <div
        class="mt-5 flex flex-wrap gap-4 border-t border-default pt-4 text-sm text-muted"
      >
        <span
          ><i
            class="mr-2 inline-block size-3 rounded-full bg-success"
          />Aprovuar</span
        ><span
          ><i class="mr-2 inline-block size-3 rounded-full bg-warning" />Në
          pritje</span
        ><span
          ><i
            class="mr-2 inline-block size-3 rounded-full bg-error"
          />Refuzuar</span
        >
      </div>
    </UCard>
      ...
      <template #body>
        <div v-if="selectedRequest" class="space-y-3 text-sm">
          <div class="flex justify-between gap-4">
            <span class="text-muted">Punëtori</span
            ><strong class="text-highlighted">{{
              selectedRequest.employee?.full_name || "Punëtori"
            }}</strong>
          </div>
          <div class="flex justify-between gap-4">
            <span class="text-muted">Lloji</span
            ><strong class="text-highlighted">{{
              leaveTypeLabels[selectedRequest.leave_type]
            }}</strong>
          </div>
          <div class="flex justify-between gap-4">
            <span class="text-muted">Periudha</span
            ><strong class="text-highlighted"
              >{{ selectedRequest.start_date }} –
              {{ selectedRequest.end_date }}</strong
            >
          </div>
          <div class="flex justify-between gap-4">
            ...
            ><UBadge
              ...
              "
              variant="subtle"
              >{{ statusLabel(selectedRequest.status) }}</UBadge
            >
          </div>
          <div
          ...
          >
            <p class="text-muted">Arsyeja</p>
            <p class="mt-1 text-highlighted">{{ selectedRequest.reason }}</p>
          </div>
          <div
            v-if="selectedRequest.rejection_reason"
            class="border-t border-default pt-3"
          >
            <p class="text-muted">Arsyeja e refuzimit</p>
    ...
    <UModal
      ...
      title="Shto festë zyrtare"
      description="Zgjidh datën dhe shkruaj emrin e festës."
    >
      <template #body>
        <form class="space-y-4" @submit.prevent="addHoliday">
          ...
          /></UFormField>
          <UFormField label="Emri i festës" required
            ><UInput
          ...
          <div class="flex justify-end gap-2">
            ...
            /><UButton
              ...
              :disabled="!holidayForm.name.trim() || !holidayForm.date"
            />
          </div>
        </form>
      </template>
    ...
    <UModal
      v-model:open="holidayListModalOpen"
      title="Lista e festave zyrtare"
    ...
    >
      <template #body>
        <div v-if="holidays.length" class="divide-y divide-default">
          ...
          >
            <div>
              <p class="font-medium text-highlighted">{{ holiday.name }}</p>
              <p class="text-sm text-muted">{{ holiday.holiday_date }}</p>
            </div>
            <div class="flex gap-2">
              ...
              /><UButton
                ...
                color="error"
                variant="soft"
                @click="openDeleteHoliday(holiday)"
              />
            </div>
          </div>
        </div>
        <UEmpty
          ...
          title="Nuk ka festa zyrtare"
          description="Shto festën e parë zyrtare nga butoni sipër."
        />
      </template>
    </UModal>

    <UModal
      v-model:open="holidayDeleteModalOpen"
      title="Fshi festën zyrtare"
      description="Ky veprim nuk mund të zhbëhet."
    >
      <template #body>
        <p class="text-sm text-muted">
          A dëshiron ta fshish festën
          <strong class="text-highlighted">{{ selectedHoliday?.name }}</strong
          >?
        </p>
        <div class="mt-5 flex justify-end gap-2">
          <UButton
            ...
            color="neutral"
            variant="ghost"
            @click="holidayDeleteModalOpen = false"
          /><UButton
            label="Po, fshije"
            color="error"
            :loading="holidaySaving"
