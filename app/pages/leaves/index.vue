...
function leaveDays(request: LeaveRequest) {
  ...
  while (current < end) {
    ...
    if (
      weekday !== 0 &&
      weekday !== 6 &&
      !officialHolidayDates.value.has(dateKey)
    )
      days++;
...
const balances = computed(() =>
  leaveProfiles.value.map((profile) => {
    const approved = requests.value.filter(
      (request) =>
        request.employee_id === profile.id && request.status === "approved",
    );
    const annualUsed = approved
      .filter((request) => request.leave_type === "annual")
      .reduce((sum, request) => sum + leaveDays(request), 0);
    const sickUsed = approved
      .filter((request) => request.leave_type === "sick")
      .reduce((sum, request) => sum + leaveDays(request), 0);
    return {
      ...profile,
      annualUsed,
...
async function load() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("role, email, phone, full_name, contract_date, annual_leave_days")
  ...
  const { data, error } = await supabase
    .from("leave_requests")
    .select(
      "id, employee_id, email, phone, leave_type, start_date, end_date, reason, rejection_reason, medical_certificate_path, status, employee:profiles!leave_requests_employee_id_fkey(full_name)",
    )
  ...
  const { data: holidays, error: holidaysError } = await supabase
    ...
    .select("holiday_date");
  if (holidaysError && holidaysError.code !== "42P01")
    errorMessage.value = holidaysError.message;
  officialHolidayDates.value = new Set(
    ((holidays || []) as OfficialHoliday[]).map(
  ...
  if (canReview.value) {
    const { data: profiles, error: profilesError } = await supabase
      .from("profiles")
      .select("id, full_name, contract_date, annual_leave_days, role")
      .order("full_name");
    if (profilesError) errorMessage.value = profilesError.message;
    leaveProfiles.value = (profiles || []) as LeaveProfile[];
  } else {
    leaveProfiles.value = profile
      ? [{ ...profile, id: currentUser.id } as LeaveProfile]
      : [];
  }
  loading.value = false;
}
...
async function submitRequest() {
  ...
  if (form.leaveType === "sick" && !medicalCertificate.value) {
    errorMessage.value =
      "Për pushimin mjekësor duhet të ngarkosh vërtetimin e mjekut.";
    return;
  }
  ...
  const overlapsApprovedLeave = (approvedLeaves || []).some(
    (leave) =>
  ...
  try {
    ...
    if (medicalCertificate.value)
      payload.append("medicalCertificate", medicalCertificate.value);
...
async function reviewRequest(
  ...
  rejectionReasonValue: string | null = null,
) {
  errorMessage.value = "";
  successMessage.value = "";
  ...
  let error: any = null;
  try {
    await $fetch(`/api/leaves/${request.id}/review`, {
      method: "PATCH",
      body: { status, rejectionReason: rejectionReasonValue },
    });
  ...
  if (error) {
    const message =
      error?.statusMessage || error?.message || "Kërkesa nuk u shqyrtua.";
    ...
    successMessage.value =
      status === "approved"
        ? "Kërkesa u aprovua dhe u ruajt."
        : "Kërkesa u refuzua dhe u ruajt.";
    await load();
  }
}
...
async function viewMedicalCertificate(request: LeaveRequest) {
  ...
  if (error || !data?.signedUrl) {
    errorMessage.value =
      error?.message || "Dëshmia mjekësore nuk mund të hapet.";
    return;
  }
...
<template>
  <section class="space-y-6">
    ...
    >
      <div>
        <h2 class="text-2xl font-semibold text-highlighted">Pushimet</h2>
        <p class="muted">Apliko për pushim dhe shqyrto kërkesat e ekipit.</p>
      </div>
      <div class="flex flex-wrap gap-3">
        <UButton
          v-if="canReview"
          to="/leaves/calendar"
          label="Kalendari i pushimeve"
          icon="i-lucide-calendar-range"
          color="neutral"
          variant="soft"
        /><UButton
          v-if="canReview"
          label="Ditët e pushimit"
          icon="i-lucide-table"
          color="neutral"
          variant="soft"
          @click="balancesModalOpen = true"
        /><UButton
          label="Apliko për pushim"
          icon="i-lucide-plus"
          @click="openModal"
    ...
    <UInput
      ...
      placeholder="Kërko punëtorin sipas emrit..."
      class="w-1/2"
    />
        ...
        <table class="w-full min-w-[900px] text-left text-sm">
          <thead class="border-b border-default text-xs uppercase text-muted">
            <tr>
              ...
              <th class="px-4 py-3">Periudha</th>
              <th class="px-4 py-3">Arsyeja</th>
              <th class="px-4 py-3">Dëshmia mjekësore</th>
              <th class="px-4 py-3">Statusi</th>
              <th v-if="canReview" class="px-4 py-3">Veprime</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              <td class="px-4 py-4 font-medium text-highlighted">
                {{ request.employee?.full_name || "Punëtori" }}
              </td>
              ...
              <td class="max-w-xs px-4 py-4 text-muted">
                ...
                ><span
                  ...
                  >Refuzim: {{ request.rejection_reason }}</span
                >
              </td>
              <td class="px-4 py-4">
                <UButton
                  v-if="request.medical_certificate_path"
                  size="xs"
                  icon="i-lucide-file-image"
                  label="Shiko dëshminë"
                  color="neutral"
                  variant="soft"
                  :loading="certificateLoadingId === request.id"
                  @click="viewMedicalCertificate(request)"
              ...
              <td class="px-4 py-4">
                <UBadge
                  ...
                  "
                  variant="subtle"
                  >{{ statusLabel(request.status) }}</UBadge
                >
              </td>
              <td v-if="canReview" class="px-4 py-4">
                <div v-if="request.status === 'pending'" class="flex gap-2">
                  <UButton
                    size="xs"
                    color="success"
                    label="Aprovo"
                    :loading="reviewSavingId === request.id"
    ...
    <UModal
      v-if="canReview"
      v-model:open="balancesModalOpen"
      title="Ditët e mbetura të pushimit"
      ...
      <template #body>
        <div class="overflow-x-auto rounded-lg border border-default">
          <table class="w-full min-w-[850px] text-left text-sm">
            <thead
              class="border-b border-default bg-elevated text-xs uppercase text-muted"
            >
              <tr>
                <th class="px-4 py-3">Punëtori</th>
                <th class="px-4 py-3">Roli</th>
                <th class="px-4 py-3">Data e kontratës</th>
                <th class="px-4 py-3">Vjetor total</th>
                <th class="px-4 py-3">Vjetor i përdorur</th>
                <th class="px-4 py-3">Vjetor i mbetur</th>
                <th class="px-4 py-3">Mjekësor i përdorur</th>
                <th class="px-4 py-3">Mjekësor i mbetur</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                ...
                <td class="px-4 py-4 text-muted">
                  {{ balance.contract_date || "—" }}
                </td>
                <td class="px-4 py-4 text-muted">
                  {{ balance.annual_leave_days }} ditë
                </td>
                <td class="px-4 py-4 text-muted">
                  {{ balance.annualUsed }} ditë
                </td>
                <td class="px-4 py-4 font-semibold text-primary">
                  {{ balance.annualRemaining }} ditë
                </td>
                <td class="px-4 py-4 text-muted">
                  {{ balance.sickUsed }} ditë
                </td>
                <td class="px-4 py-4 font-semibold text-success">
                  {{ balance.sickRemaining }} ditë
                </td>
              </tr>
        ...
        <p class="mt-4 text-sm text-muted">
          Pushimi mjekësor është i caktuar në mënyrë statike në 20 ditë për çdo
          përdorues.
        </p>
      </template>
    </UModal>
    ...
    <UModal
      ...
      title="Apliko për pushim"
      description="Kërkesa do t'i dërgohet menaxherit për shqyrtim."
    >
      <template #body
        ><form class="space-y-4" @submit.prevent="submitRequest">
          <UFormField label="Lloji i pushimit"
            ><USelect
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <UFormField label="Nga" required
              ><UInput
                v-model="form.startDate"
                type="date"
                class="w-full" /></UFormField
            ><UFormField label="Deri" required
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <UFormField label="Email" required
              ><UInput
                v-model="form.email"
                type="email"
                placeholder="email@atomx-solutions.com"
                class="w-full" /></UFormField
            ><UFormField label="Nr. i telefonit"
              ><UInput
                v-model="form.phone"
                type="tel"
                placeholder="+383..."
          ...
          <UFormField label="Arsyeja"
            ><UTextarea
              v-model="form.reason"
              placeholder="Shkruaj arsyen e pushimit..."
              class="w-full"
          /></UFormField>
          <UFormField
            v-if="form.leaveType === 'sick'"
            label="Vërtetimi mjekësor"
            description="Ngarko foton ose PDF-in e lëshuar nga mjeku."
            required
          >
            <input
              type="file"
              accept="image/*,.pdf"
              required
              class="block w-full rounded-md border border-default bg-default px-3 py-2 text-sm text-highlighted file:mr-3 file:rounded file:border-0 file:bg-primary file:px-3 file:py-1 file:text-white"
              @change="onMedicalCertificateChange"
            />
          </UFormField>
          <div class="flex justify-end gap-3 pt-3">
            ...
            /><UButton type="submit" label="Dërgo kërkesën" :loading="saving" />
          </div></form
      ></template>
    </UModal>

    <UModal
      v-model:open="rejectionModalOpen"
      title="Refuzo kërkesën"
      description="Shkruaj arsyen pse kjo kërkesë po refuzohet."
    >
      <template #body>
        <form class="space-y-4" @submit.prevent="rejectRequest">
          ...
          <div class="flex justify-end gap-3">
            ...
            <UButton
              type="submit"
              color="error"
              label="Ruaj refuzimin"
              :loading="reviewSavingId !== null"
            />
          </div>
        </form>
      </template>
