...
function workMinutes(record: Attendance | null) {
  ...
  let minutes =
    (Date.parse(record.check_out) - Date.parse(record.check_in)) / 60000;
  ...
  const standardBreakMinutes = Math.max(
    0,
    Number(companySettings.value.break_minutes) || 0,
  );
  let breakMinutes = standardBreakMinutes;
  if (record.break_out && record.break_in) {
    const actualBreakMinutes =
      (Date.parse(record.break_in) - Date.parse(record.break_out)) / 60000;
...
function lateMinutes(record: Attendance | null) {
  ...
  const [hours, minutes] = companySettings.value.work_start_time
    .slice(0, 5)
    .split(":")
    .map(Number);
  ...
  return Math.max(
    0,
    Math.floor((actual.getTime() - planned.getTime()) / 60000),
  );
}
...
function lateLabel(record: Attendance | null) {
  ...
  return minutes && minutes > 0
    ? `Vonesë ${formatTotalMinutes(minutes)}`
    : "Në kohë";
}
...
const monthlySummary = computed(() => {
  ...
  for (const record of monthlyRecords.value) {
    const item = summary.get(record.employee_id) || {
...
async function loadTeamRecords() {
  let query = supabase
    .from("attendance")
    .select(
      "id, employee_id, work_date, check_in, break_out, break_in, check_out, notes, employee:profiles!attendance_employee_id_fkey(full_name, role)",
    )
...
async function loadMonthlyRecords() {
  ...
  if (monthlyFilters.employeeId !== "all")
    query = query.eq("employee_id", monthlyFilters.employeeId);
  const { data, error } = await query;
  if (error) throw error;
...
async function load() {
  ...
  try {
    ...
    const { data: settings, error: settingsError } = await supabase
      ...
      .select("work_start_time, work_end_time, break_minutes")
      .eq("id", 1)
      .maybeSingle();
    if (settingsError && !settingsError.message.includes("company_settings"))
      throw settingsError;
    if (settings) companySettings.value = settings as CompanySettings;
    const { data: own, error: ownError } = await supabase
      .from("attendance")
      .select(
        "id, employee_id, work_date, check_in, break_out, break_in, check_out, notes",
      )
...
watch([() => filters.workDate, () => filters.employeeId], async () => {
...
<template>
  <section class="space-y-6">
    ...
    <UCard>
      ...
      <div v-else class="space-y-6">
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <div>
            <p class="text-xs uppercase text-muted">Hyrja në punë</p>
            <p class="mt-1 text-xl font-semibold text-highlighted">
              {{ time(current?.check_in || null) }}
            </p>
          </div>
          <div>
            <p class="text-xs uppercase text-muted">Dalja në pauzë</p>
            <p class="mt-1 text-xl font-semibold text-highlighted">
              {{ time(current?.break_out || null) }}
            </p>
          </div>
          <div>
            <p class="text-xs uppercase text-muted">Kthimi nga pauza</p>
            <p class="mt-1 text-xl font-semibold text-highlighted">
              {{ time(current?.break_in || null) }}
            </p>
          </div>
          <div>
            <p class="text-xs uppercase text-muted">Dalja nga puna</p>
            <p class="mt-1 text-xl font-semibold text-highlighted">
        ...
        <div
          class="flex flex-wrap items-center justify-between gap-3 border-t border-default pt-5"
        >
          <div
            v-if="lateMinutes(current) && lateMinutes(current)! > 0"
            class="flex items-center gap-1 text-sm font-medium text-warning"
          >
          ...
          <div class="flex flex-wrap gap-3">
            ...
            />
            <UButton
              v-else-if="!current.break_out"
              label="Dalja në pauzë"
              icon="i-lucide-coffee"
              color="neutral"
              :loading="saving"
              @click="setTime('break_out')"
            />
            <UButton
              v-else-if="!current.break_in"
              label="Kthimi nga pauza"
              icon="i-lucide-play"
              :loading="saving"
              @click="setTime('break_in')"
            />
            <UButton
              v-else-if="!current.check_out"
              label="Dalja nga puna"
              icon="i-lucide-log-out"
              color="neutral"
              :loading="saving"
              @click="setTime('check_out')"
            />
            <UBadge v-else color="success" variant="subtle" class="px-4 py-2"
              >Dita u kompletua</UBadge
            >
          </div>
        </div>
      </div>
    ...
    <UModal
      ...
      title="Hyrjet dhe daljet e punëtorëve"
        ...
        <div class="space-y-5">
          <div class="flex flex-wrap items-start gap-4">
            <UFormField label="Data"
              ><UInput v-model="filters.workDate" type="date"
            /></UFormField>
            <UFormField label="Punëtori"
              ><USelect v-model="filters.employeeId" :items="employeeItems"
            /></UFormField>
          </div>
          <div
            class="max-h-[55vh] overflow-auto rounded-lg border border-default"
          >
            <table class="w-full min-w-[1050px] text-left text-sm">
              <thead
                class="sticky top-0 border-b border-default bg-elevated text-xs uppercase text-muted"
              >
                <tr>
                  <th class="px-4 py-3">Punëtori</th>
                  <th class="px-4 py-3">Data</th>
                  <th class="px-4 py-3">Hyrja</th>
                  <th class="px-4 py-3">Dalja në pauzë</th>
                  <th class="px-4 py-3">Kthimi</th>
                  <th class="px-4 py-3">Dalja nga puna</th>
                  <th class="px-4 py-3">Orari i punës</th>
                  <th class="px-4 py-3">Orë shtesë</th>
                </tr>
              </thead>
              <tbody>
                <tr
                ...
                >
                  ...
                  <td class="px-4 py-4 text-muted">{{ record.work_date }}</td>
                  <td class="px-4 py-4 text-muted">
                    {{ time(record.check_in) }}
                  </td>
                  <td class="px-4 py-4 text-muted">
                    {{ time(record.break_out) }}
                  </td>
                  <td class="px-4 py-4 text-muted">
                    {{ time(record.break_in) }}
                  </td>
                  <td class="px-4 py-4 text-muted">
    ...
    <UModal
      v-model:open="monthlyModalOpen"
      title="Raporti mujor i orëve"
      :ui="{ content: 'max-w-5xl' }"
      @update:open="
        (open) => {
          if (open) loadMonthlyRecords();
        }
      "
    >
      <template #body>
        <div class="space-y-5">
          <div class="flex flex-wrap items-end gap-4">
            <UFormField label="Muaji"
              ><UInput v-model="monthlyFilters.month" type="month"
            /></UFormField>
            <UFormField label="Punëtori"
              ><USelect
                v-model="monthlyFilters.employeeId"
                :items="employeeItems"
            /></UFormField>
          </div>
          <UAlert
            ...
            color="neutral"
            variant="subtle"
            title="Nuk ka regjistrime për këtë periudhë."
          />
          <div v-else class="overflow-auto rounded-lg border border-default">
            <table class="w-full min-w-[720px] text-left text-sm">
              ...
              >
                <tr>
                  <th class="px-4 py-3">Punëtori</th>
                  <th class="px-4 py-3">Ditë pune</th>
                  <th class="px-4 py-3">Orë pune</th>
                  <th class="px-4 py-3">Orë shtesë</th>
                </tr>
              </thead>
              <tbody>
                ...
                >
                  ...
                  <td class="px-4 py-4 text-muted">{{ item.days }}</td>
                  <td class="px-4 py-4 text-highlighted">
                    {{ formatTotalMinutes(item.workMinutes) }}
                  </td>
                  <td class="px-4 py-4 font-semibold text-primary">
                    {{ formatTotalMinutes(item.overtimeMinutes) }}
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <p class="text-xs text-muted">
            Orët shtesë llogariten vetëm për kohën reale të punës mbi 8 orë në
            ditë, pas zbritjes së pauzës.
          </p>
        </div>
      </template>
    </UModal>
