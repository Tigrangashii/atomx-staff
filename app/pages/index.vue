...
type Entry = { id: string; check_in: string | null; employee?: { full_name: string } | null }
type PendingLeave = { id: string; start_date: string; end_date: string; employee?: { full_name: string } | null }
...
const isReviewer = computed(() => role.value === 'owner' || role.value === 'manager')
const canSeeDashboard = computed(() => isReviewer.value || hasCheckedIn.value)
const currentDateLabel = computed(() => currentDateTime.value.toLocaleDateString('sq-AL'))
const currentTimeLabel = computed(() => currentDateTime.value.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }))
const stats = computed(() => isReviewer.value
  ? [
      { label: 'Punëtorë aktivë', amount: activeStaffCount.value, detail: 'Në ekip' },
      { label: 'Pushime në pritje', amount: pendingLeaveCount.value, detail: 'Për shqyrtim' },
      { label: 'Hyrje sot', amount: todayAttendanceCount.value, detail: 'Regjistrime' },
      { label: 'Raporte të reja', amount: todayReportsCount.value, detail: 'Të dorëzuara sot' }
    ]
  : [
      { label: 'Hyrja sot', amount: userEntryTime.value, detail: 'Ora e regjistrimit' },
      { label: 'Kërkesa për pushim', amount: myPendingLeaveCount.value, detail: 'Në pritje' }
    ])

async function countRows(table: 'profiles' | 'leave_requests' | 'attendance' | 'daily_reports', filters: Array<[string, string, string]> = []) {
...
async function loadDashboard() {
  ...
  try {
    ...
    fullName.value = profile?.full_name?.trim() || authUser.user_metadata?.full_name || authUser.email?.split('@')[0] || 'përdorues'
    role.value = (profile?.role || 'user') as Role

    const { data: ownAttendance, error: attendanceError } = await supabase.from('attendance').select('check_in').eq('employee_id', authUser.id).eq('work_date', today).maybeSingle()
    if (attendanceError) throw attendanceError
    hasCheckedIn.value = Boolean(ownAttendance?.check_in)

    if (isReviewer.value) {
      const [activeStaff, pendingLeaveTotal, todayAttendance, submittedReports] = await Promise.all([
        countRows('profiles', [['is_active', 'eq', 'true']]),
        countRows('leave_requests', [['status', 'eq', 'pending']]),
        countRows('attendance', [['work_date', 'eq', today], ['check_in', 'gte', '1900-01-01']]),
      ...
      const [{ data: entries }, { data: leaves }] = await Promise.all([
        supabase.from('attendance').select('id, check_in, employee:profiles!attendance_employee_id_fkey(full_name)').eq('work_date', today).not('check_in', 'is', null).order('check_in', { ascending: false }).limit(8),
        supabase.from('leave_requests').select('id, start_date, end_date, employee:profiles!leave_requests_employee_id_fkey(full_name)').eq('status', 'pending').order('created_at', { ascending: false }).limit(8)
      ])
      ...
      const [myPendingLeaves, todayReports] = await Promise.all([
        countRows('leave_requests', [['employee_id', 'eq', authUser.id], ['status', 'eq', 'pending']]),
        countRows('daily_reports', [['employee_id', 'eq', authUser.id], ['report_date', 'eq', today]])
      ])
      userEntryTime.value = ownAttendance?.check_in ? new Date(ownAttendance.check_in).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : '—'
...
onMounted(() => {
  clockTimer = setInterval(() => { currentDateTime.value = new Date() }, 1000)
  loadDashboard()
})
onBeforeUnmount(() => {
...
<template>
  <section class="dashboard-page">
    ...
    <UCard v-else class="mt-6"><div class="flex flex-col items-center gap-4 py-10 text-center"><UIcon name="i-lucide-clock-3" class="size-10 text-primary" /><h3 class="text-lg font-semibold text-highlighted">Regjistro hyrjen në punë</h3><p class="max-w-md text-sm text-muted">Dashboard-i dhe të dhënat personale do të shfaqen pasi të regjistrosh hyrjen e sotme.</p><UButton to="/attendance" label="Shko te Hyrje / Dalje" icon="i-lucide-arrow-right" /></div></UCard>
  </section>
</template>
