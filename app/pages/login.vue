...
onMounted(() => {
  ...
  if (typeof confirmationUrl === "string" && confirmationUrl) {
    window.location.replace(
      `/auth/confirm?confirmation_url=${encodeURIComponent(confirmationUrl)}`,
    );
    return;
  }
...
<template>
  <main
    class="relative flex min-h-screen items-center justify-center overflow-hidden bg-[url(/images/background.png)] bg-cover bg-center bg-no-repeat px-4 py-12"
  >
    <UCard
      class="relative z-10 w-full max-w-md shadow-sm"
      :ui="{ body: 'p-8 sm:p-10' }"
    >
      <div class="mb-8 text-center">
        <img
          class="mx-auto mb-4 h-24 w-48 object-contain"
        ...
        <p class="mt-2 text-sm text-slate-500">
          Kyçu në panelin e menaxhimit të stafit
        </p>
      </div>
      ...
      <form class="space-y-5" @submit.prevent="login">
        <UFormField label="Email" name="email">
          <UInput
            v-model="email"
            type="email"
            placeholder="emri@atomx-solutions.com"
            class="w-full"
            required
          />
        </UFormField>
      ...
      <p class="mt-8 text-center text-xs text-slate-400">
        AtomX Solutions · Staff Management
      </p>
    </UCard>
  </main>
</template>
