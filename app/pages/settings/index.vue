...
const canManageDocuments = computed(
  () => profile.value?.role === "owner" || profile.value?.role === "manager",
);
const activeTab = computed(() =>
  route.query.tab === "company" ? "company" : "profile",
);
...
async function load() {
  ...
  const [
    ...
    supabase
      ...
      .select(
        "id, title, description, file_name, storage_path, mime_type, file_size, created_at",
      )
      .order("created_at", { ascending: false }),
    supabase
      .from("company_settings")
      .select("id, company_name, work_start_time, work_end_time, break_minutes")
      .eq("id", 1)
      .maybeSingle(),
  ]);
  ...
  companyForm.workStartTime =
    companyData?.work_start_time?.slice(0, 5) || "08:00";
...
async function saveCompanySettings() {
  ...
  if (error) errorMessage.value = error.message;
    ...
    successMessage.value = "Orari i kompanisë u ruajt me sukses.";
    await load();
  }
}
...
function onFileChange(event: Event) {
  ...
  if (selectedFile.value && !documentForm.title)
    documentForm.title = selectedFile.value.name.replace(/\.[^/.]+$/, "");
}
...
async function downloadDocument(documentItem: CompanyDocument) {
  const { data, error } = await supabase.storage
    .from("company-documents")
    .createSignedUrl(documentItem.storage_path, 60 * 5, {
      download: documentItem.file_name,
    });
  if (error) errorMessage.value = error.message;
...
async function deleteDocument(documentItem: CompanyDocument) {
  if (
    !window.confirm(`A dëshiron ta fshish dokumentin "${documentItem.title}"?`)
  )
    return;
  errorMessage.value = "";
  const { error: storageError } = await supabase.storage
    .from("company-documents")
    .remove([documentItem.storage_path]);
  if (storageError) {
    errorMessage.value = storageError.message;
    return;
  }
  const { error } = await supabase
    .from("company_documents")
  ...
  if (error) errorMessage.value = error.message;
    ...
    successMessage.value = "Dokumenti u fshi.";
    await load();
  }
}
...
<template>
  <section class="space-y-6">
    <div>
      <h2 class="text-2xl font-semibold text-highlighted">Settings</h2>
      <p class="muted">
        Menaxho detajet e profilit dhe dokumentet e kompanisë.
      </p>
    </div>
    <UAlert
      v-if="successMessage"
    ...
    <div v-if="activeTab === 'profile'">
      <UCard>
        <template #header
          ><h3 class="font-semibold text-highlighted">Detajet e profilit</h3>
          <p class="mt-1 text-sm text-muted">
            Këto të dhëna përdoren në aplikacion.
          </p></template
        >
        <form class="space-y-4" @submit.prevent="saveProfile">
          ...
          <UFormField label="Roli"
            ><UInput :model-value="profile?.role || ''" disabled class="w-full"
          /></UFormField>
    ...
    <div v-else class="space-y-6">
      <UCard>
        ...
        >
          <UFormField label="Emri i kompanisë"
            ><UInput
          ...
          /></UFormField>
          <div />
          <UFormField label="Fillimi i orarit"
            ><UInput
              v-model="companyForm.workStartTime"
              type="time"
              :disabled="!canManageDocuments"
              class="w-full"
          /></UFormField>
          <UFormField label="Përfundimi i orarit"
            ><UInput
              v-model="companyForm.workEndTime"
              type="time"
              :disabled="!canManageDocuments"
              class="w-full"
          /></UFormField>
          <UFormField label="Pauza e planifikuar (minuta)"
            ><UInput
              v-model.number="companyForm.breakMinutes"
              type="number"
              min="0"
              max="480"
              :disabled="!canManageDocuments"
              class="w-full"
          /></UFormField>
          <div v-if="canManageDocuments" class="flex justify-end md:col-span-2">
            <UButton
              type="submit"
              label="Ruaj orarin"
      ...
      <UCard v-if="canManageDocuments">
        <template #header
          ...
          <p class="mt-1 text-sm text-muted">
            Politika, kontrata dhe dokumente të rëndësishme.
          </p></template
        >
        <form class="space-y-4" @submit.prevent="uploadDocument">
          <UFormField label="Titulli i dokumentit" required
            ><UInput
              v-model="documentForm.title"
              placeholder="p.sh. Politikat e Brendshme të Kompanisë"
              class="w-full"
          /></UFormField>
          <UFormField label="Përshkrimi"
            ><UTextarea
              ...
              class="w-full"
          /></UFormField>
          <UFormField label="Dokumenti" required
            ><input
      ...
      <UCard>
        ...
        <div v-else-if="!documents.length" class="py-10 text-center text-muted">
          ...
          >
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0">
                ...
                <p class="truncate text-sm text-muted">
                  {{ documentItem.description || documentItem.file_name }} ·
                  {{ formatSize(documentItem.file_size) }}
                </p>
              </div>
            </div>
            <div class="flex shrink-0 gap-2">
              <UButton
                ...
                icon="i-lucide-eye"
                label="Shiko"
                @click="viewDocument(documentItem)"
              /><UButton
                size="sm"
                variant="soft"
                icon="i-lucide-download"
                label="Shkarko"
                @click="downloadDocument(documentItem)"
              /><UButton
                ...
                aria-label="Fshi dokumentin"
                @click="deleteDocument(documentItem)"
              />
            </div>
          </div>
        </div>
