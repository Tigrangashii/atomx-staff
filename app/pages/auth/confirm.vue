...
async function checkInviteSession() {
  ...
  if (typeof tokenHash === "string" && tokenHash) {
    ...
    if (
      ...
      ["invite", "recovery", "email_change", "magiclink", "signup"].includes(
        tokenType,
      )
    ) {
  ...
  if (typeof confirmationUrl === "string" && confirmationUrl) {
    ...
    if (
      ...
      !confirmationUrl.includes("redirect_to=")
    ) {
  ...
  if (data.session) {
    ...
    errorMessage.value =
      "Ftesa është e pavlefshme ose ka skaduar. Kërko një ftesë të re.";
  }
...
async function setPassword() {
  ...
  if (inviteTokenHash.value) {
    ...
    if (verifyError) {
      saving.value = false;
      errorMessage.value =
        "Ftesa ka skaduar ose është përdorur. Kërko një ftesë të re.";
      return;
    }
  }
    ...
    <UCard class="w-full max-w-md shadow-sm" :ui="{ body: 'p-8 sm:p-10' }">
      <div class="mb-8 text-center">
        ...
        <p class="mt-2 text-sm text-slate-500">
          Vendose fjalëkalimin për AtomX Staff
        </p>
      </div>

      <div v-if="loading" class="py-6 text-center text-sm text-slate-500">
        Duke verifikuar ftesën...
      </div>
      <UAlert
        v-else-if="errorMessage"
        color="error"
        variant="subtle"
        title="Ftesa nuk mund të përdoret"
