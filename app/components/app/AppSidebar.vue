...
const navigation = computed(() => [
  ...
  ...(role.value === 'owner' || role.value === 'manager' ? [{ label: 'Stafi', icon: 'i-lucide-users', to: '/staff' }] : []),
  { label: 'Pushimet', icon: 'i-lucide-calendar-days', to: '/leaves' },
  { label: 'Raport ditor', icon: 'i-lucide-clipboard-list', to: '/reports' },
  ...(role.value === 'owner' || role.value === 'manager' ? [{ label: 'Raportet e stafit', icon: 'i-lucide-clipboard-check', to: '/reports/team' }] : []),
  ...(role.value === 'owner' ? [{ label: 'Projektet', icon: 'i-lucide-folder-kanban', to: '/projects' }] : []),
    ...
    <template #default="{ collapsed }">
      <nav class="grid gap-1" aria-label="Navigimi kryesor">
        ...
        >
          ...
          <UIcon v-if="!collapsed" :name="settingsOpen ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" class="size-4" />
        </button>

        <div v-if="settingsOpen && !collapsed" class="ml-4 grid gap-1 border-l border-default pl-3">
          <NuxtLink to="/settings?tab=profile" class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-muted transition-colors hover:bg-elevated hover:text-highlighted" :class="{ 'bg-primary/10 font-semibold text-primary': $route.query.tab !== 'company' }">
            <UIcon name="i-lucide-user-round" class="size-5 shrink-0" />
            <span>Profile</span>
          </NuxtLink>
          <NuxtLink to="/settings?tab=company" class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-muted transition-colors hover:bg-elevated hover:text-highlighted" :class="{ 'bg-primary/10 font-semibold text-primary': $route.query.tab === 'company' }">
